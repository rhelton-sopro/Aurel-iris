"use client"

import * as React from "react"
import { Checkbox as CheckboxPrimitive } from "@base-ui/react/checkbox"
import { CheckIcon } from "lucide-react"

import { cn } from "@/lib/utils"

/**
 * Checkbox — base-ui primitive wrapper following the same pattern as Select/Input.
 *
 * Added in Plan 07.4-07b to support BilateralFindingsEditor (Checkbox +
 * Textarea always-render per UI-SPEC FLAG-5). No prior consumer in the app.
 *
 * API mirrors base-ui Root: { checked, onCheckedChange, disabled, id }.
 */
function Checkbox({
  className,
  ...props
}: CheckboxPrimitive.Root.Props) {
  return (
    <CheckboxPrimitive.Root
      data-slot="checkbox"
      className={cn(
        "peer inline-flex size-4 shrink-0 items-center justify-center rounded-none border border-input bg-transparent text-current outline-none transition-colors focus-visible:border-teal disabled:cursor-not-allowed disabled:opacity-50 data-[checked]:border-teal-dark data-[checked]:bg-teal-dark data-[checked]:text-white",
        className,
      )}
      {...props}
    >
      <CheckboxPrimitive.Indicator className="flex items-center justify-center text-current">
        <CheckIcon className="size-3.5" />
      </CheckboxPrimitive.Indicator>
    </CheckboxPrimitive.Root>
  )
}

export { Checkbox }
