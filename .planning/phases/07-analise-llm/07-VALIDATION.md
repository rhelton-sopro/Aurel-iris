...
- [ ] `supabase/migrations/__tests__/0007_smoke.sql` (or pgTAP equiv) — jsonb_concat_sections_pt_br orders 1_..13_ correctly (NOT lex order: `13_` before `2_`), GENERATED columns recompute on jsonb update, IMMUTABLE PARALLEL SAFE flags set
- [ ] `apps/web/scripts/__tests__/audit-vocabulary.test.mjs` — DIRS includes `lib/anthropic`, `prompts/` allowlist marker honored
- [ ] `apps/web/lib/anthropic/__tests__/integration.test.ts` (skip-by-default; opt-in via `pnpm test:llm`) — end-to-end Anthropic call against fixture features → asserts 14 sections received, encerramento appended, anchor_rate computed, vocab clean
...
| Disclaimer literal SPEC §6 lines 622-628 always present | LLM-03 / SC4 | Server-side append guarantees presence, but full literal match needs visual check | Open 3 generations in editor; verify disclaimer matches SPEC §6 word-for-word, not editable, collapsed by default |
...
- [ ] Wave 0 covers all MISSING references (14 test files above)
