...
- **7.2-A** Add findings-hierarchy clause to LLM system prompt — visual prominence drives section ordering, not just feature presence. Concrete weight examples (founder-confirmed via Gemini cross-validation 2026-05-12): "heterocromia central pigmentada" > "sectoral lacunas grau 1"; "dense pigmented halo around pupil with heterocromia=true + multi-pigment array" → lead finding in §3 + §9. Founder UAT showed digestive halo underweighted in §3/§9 ordering. Scope: `apps/web/prompts/system.md` + SPEC.md §6 (D-PR1 frozen contract) coord. NOT 7.2 Modal scope strictly — independent prompt-eng item, but groups with 7.2 deploy cycle.
- **7.2-B** Tune Modal lacuna size measurement to exclude chromatic transition zones from extent calculation. Founder UAT: thyroid lacuna at hour-4 reported 24-27mm bilateral when direct visual shows smaller signals; Modal counts chromatic transition zone as part of extent. Scope: `vision-service/pipeline/segment.py` (or equivalent).
...
Item from Gemini list (LLM findings-hierarchy by visual prominence) is the same concern as 7.2-A — kept consolidated under 7.2-A with refined weight examples.
