...
- [ ] **Fase 11.1 (INSERTED, NEW 2026-05-26): Invite terapeuta via signup form** — Bug descoberto durante smoke Fase 9 (09-05): `inviteTherapistAction` usa `supabase.auth.admin.generateLink({type:'invite', redirectTo:/dashboard})` mas `/dashboard` não troca PKCE code por sessão → terapeuta cai em `/login` em vez de signup form com email pré-preenchido. Fix: tabela `therapist_invites(token uuid, email, expires_at 7d, used_at)` + rewrite `inviteTherapistAction` (D-DUPE bloqueia email já cadastrado) + rota `/convite-terapeuta/[token]` (RSC lookup + form igual /signup com email read-only + OTP obrigatório). Inclui também `UNIQUE(therapist_id, email) ON clients` (resolve memory tech debt `project_clients_unique_email_tech_debt`). 3 plans. Desbloqueia smoke E2E cenários 1+2 da Fase 9.
