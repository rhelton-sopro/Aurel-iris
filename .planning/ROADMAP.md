...
> **Re-sync 2026-05-31** — Fase 8 SAIU de closeout: **Asaas LIVE em prod (go-live 2026-05-30)**, HEAD `9d71c95`, smokes E2E do founder em andamento. Sessão 05-31 adicionou 15 commits sobre o go-live (cartão habilitado, reembolso humanize+parcial D+1, bug billing 0042 crédito comprado não debitava, geração híbrida sem erro falso, gate de captura endurecido, onboarding trial órfã+welcome+consentimento). Fonte de verdade canônica = memória `project_session_2026_05_31_handoff`. Pendências ativas: testes de aceitação do founder (estorno parcial D+1, geração >5min, callback auto-retorno), reverter pricing de teste (4 SKUs a R$5), onboarding Cluster 2b (8.1).
...
- [~] **Fase 7.5 (INSERTED): Iris Codex — Tendency mapping engine — ABANDONED 2026-05-26** — Sonnet 2x Stage 1 venceu RAG-anchored/deterministic mapping na prática (founder verbatim: "isso deveria estar fora de cogitação"). Jornada RAG → Sonnet 2x mostrou que mapping engine determinístico regrediria filosoficamente. Reforçar via memory: `project_fase_7_5_abandoned_sonnet_won`.
- [~] **Fase 8: Pagamento + LGPD** — **CÓDIGO-COMPLETO, EM CLOSEOUT (2026-05-29) → AGORA LIVE EM PROD (Asaas go-live 2026-05-30, HEAD 9d71c95; código completo 15/15, verificado code-level 9/9 SC; verifier=human_needed = só smokes E2E do founder; sessão 05-31 endureceu billing/geração/captura/onboarding em 15 commits).** 14 plans entregues em 6 waves (pacote pré-pago Asaas + trial + reservas FIFO 7d + arrependimento + termo biométrico nativo + privacy/termos/deleção + audit log + copy obrigatória). Test suite Fase 8 TODA verde (112 passed em 18 arquivos); lint+tsc dos arquivos Fase 8 limpos. **PENDING-FOUNDER:** smokes E2E live (Cenários A-F), decisão de cutover prod, e limpeza dos 20 lint errors pré-existentes antes do `next build` LIVE. Checklist go-live consolidado em `.planning/phases/08-pagamento-lgpd/08-VERIFICATION.md`. Original: "Não-bloqueador pra B2B v1" (founder 2026-05-18); reaberto e executado.
...
### Fase 7.1.5 (INSERTED): Detect/segment robustness para close-up iris photos
**Origem:** Empirical investigation durante `/gsd-discuss-phase 07.2` em 2026-05-11. O probe `vision-service/scripts/probe_pigment_deltas.py` + o field novo `processing_metadata.detect_diagnostics` (deployado commit `8494a4a`) revelaram que a hipótese original ("threshold tuning resolve") era sintoma — não causa raiz. **Cadeia causal empírica:** MediaPipe FaceLandmarker falha 5/6 fotos da Nailli (no face context em close-ups) → Hough fallback é chamado → escolhe círculos errados em close-ups (bordas de pálpebra/sobrancelha) → segment.iris_mask refina sobre seed errada → **segment centers de 3 ângulos da mesma íris ficam 599-1497 px distantes** entre si (radii variam 1.7×) → `compose.photometric_combine` faz média ponderada de 3 polar-unwraps de **regiões geometricamente diferentes** → enhanced_polar é Frankenstein (visualmente: ~35% íris + ~65% pálpebra/esclera/pele) → `classify_iris_color` devolve `castanho` (centroide mais próximo do LAB-mean da pixel-soup) + `_classify_constitution` cai no fallback `hematogenea` + `detect_sectoral_pigments` retorna `[]`. Sem fix, 7.2 (corpus + threshold tuning) opera sobre lixo.
**Goal**: `detect.find_iris` + `segment.iris_mask` produzem iris circles geometricamente consistentes em close-up iris photos onde MediaPipe FaceLandmarker retorna `no_face`. Especificamente: para a Nailli (e qualquer reading com 3 ângulos), segment.iris_circle dos 3 ângulos do mesmo olho têm centros ≤50 px de distância entre si E radii dentro de ±15% de variação. Validação visual via `enhanced_polar` PNG: textura radial striated reconhecível como íris em ≥80% do width do polar (sem zonas grandes uniformemente escuras).
**Depends on**: Fase 7.1 (sectoral_pigments wire, schemas SectoralPigment + ConstitutionType expanded, `processing_metadata.detect_diagnostics` field deployado). Não depende de Fase 7 LLM nem Fase 6 RAG.
**Requirements**: nenhum formal (é remediação de pipeline interno; não cria capability nova de usuário). Backlog Phase 9 RESP-01..03 ainda válido pós-fix.
**Success Criteria** (o que deve ser verdade):
  1. `enhanced_polar` PNG da Nailli (reading `71a7bf1d-747f-4de8-9129-13b69197c6a4`) gerado pós-fix é visualmente reconhecível como íris (founder qualitative gate em `/admin/calibration/[id]` OU comparação direta com PNG pré-fix em `vision-service/scripts/output/`).
  2. `vision_features.processing_metadata.detect_diagnostics` da Nailli pós-reprocess mostra: para cada olho, segment centers dos 3 ângulos (frontal/lateral/backlight) têm distância máxima ≤50 px entre si E segment radii têm coeficiente de variação ≤15%.
  3. Pelo menos uma das hipóteses (A-F do DISCUSSION-NOTES) é investigada via research-phase + chosen approach é implementado. Não é gate qual é (founder + research-phase escolhem).
...
- **Modal cost envelope:** se approach C (U-Net) for escolhido, modelo pre-baked no Modal image (`run_commands wget`) — não download cold-start. Function timeout segue 120s.
- **`detect_diagnostics` field é o gate empírico:** todo PR de 07.1.5 inclui before/after dump do field pra Nailli no PR description.
- **Capture UX não pode regredir:** fotos close-up legítimas (íris ≥30% do frame) devem funcionar — opção E (pedir founder zoomed-out shots) está rejeitada como UX regression.

**UI hint**: no (mudança é puro vision-service; `/admin/calibration` já existe para visualizar resultados).

### Fase 7.1.6 (COMPLETE 2026-05-12 — 7/7 plans + UAT APPROVED via clinical inspection of f4408c23): Canonical Capture Pipeline
**Origem:** Closure de 07.1.5 P1 com verdict `B_INFEASIBLE` (commits a67eb1f→f415ac8, 2026-05-11) — HSV color pre-segmentation convergiu monotonicamente (974→496→279 px Cycles 1-3) mas não atingiu o gate ≤50 px. Founder + Claude investigaram alternativa ao U-Net escalation (07.1.5 P2 ABANDONED em ESCALATE_NEW_PHASE 2026-05-11). **Probe arquitetural** `apps/web/scripts/probe-haiku-iris-landmarks.mjs` testou Haiku 4.5 e Sonnet 4.6 como VLM landmark detectors: Haiku templatou (radius=0.12 em 5/6 fotos Nailli, center_x=0.58 em 4/6) — inviável; Sonnet 4.6 mediu per-image, 5/6 crops visualmente aprovados (custo $0.05/reading). **Smoking gun universal:** Sonnet templatou rotation 100% (`rotation_angle_deg=-3` em todas as 6 fotos) — VLM rotation detection morta independente do modelo. **Decisão arquitetural (2026-05-11):** 07.1.6 = canonical CROP via Sonnet bbox (substitui U-Net); sem rotation correction; terapeuta responsável pela orientação. Re-probe Nailli (reading `e85ea7de` com novo capture protocol commit `f885462`) em 2026-05-11 confirmou: LEFT max-center-spread 637→343 px (-59%) mas continua >50px → confirma per-photo canonical é necessário, não shared-bbox-per-eye. Substitui o escalation U-Net (4 ganhos vs 1: detect/segment + Sonnet Vision downstream economy + corpus standardization + per-reading audit-ability).
**Goal**: Cada uma das 6 fotos de uma leitura nova é canonicalizada server-side antes do Modal: Sonnet 4.6 retorna `iris_bbox` (center + radius em fração da menor dimensão), `sharp` faz crop com floor `max(2.5×r, 0.4×min(W,H))` + resize 800×800, salva em `{therapist}/{reading}/canonical/{eye}_{angle}.jpg`, Modal recebe canonical signed URLs em vez de originais raw. Gate de qualidade pre-Modal: cross-angle consistency (>0.08 do mediano em x ou y → outlier) + geometric sanity (center in [0.20, 0.80], radius_pct in [0.05, 0.30]); gate fail → fallback à imagem original com `canonical_status='fallback'` em metadata. Nailli nova (`e85ea7de`) reprocessada via canonical produz segment centers ≤50 px de distância entre 3 ângulos do mesmo olho (o gate empírico que 07.1.5 B_INFEASIBLE perdeu).
**Depends on**: Fase 7.1 (sectoral_pigments wire; `processing_metadata.detect_diagnostics`) + capture/validate hardening shipped commit `ed7b7be` (borrado + reflexo_total hard-block, muito_longe qualitativo) + capture protocol revisto shipped commit `f885462` (paciente fixo + flash + tilt). Não depende de Fase 6 RAG nem Fase 7 LLM. **Substitui formalmente o ESCALATE_C path do 07.1.5 P2** (ABANDONED).
**Requirements**: nenhum formal (é remediação de pipeline; não cria capability nova de usuário). Backlog Phase 9 RESP-01..03 ainda válido pós-fix.
**Success Criteria** (o que deve ser verdade):
  1. Endpoint server-side de canonicalize (apps/web) processa 6 fotos em paralelo via Sonnet 4.6 + sharp crop + Storage upload. Latency wall-clock ≤10s pra 6 fotos (warm path).
  2. Reading nova (post-deploy) com `CANONICAL_CAPTURE_ENABLED=true` produz `reading_images.canonical_storage_path` non-NULL para ≥5/6 fotos (1/6 fallback é aceitável; founder Nailli e85ea7de baseline).
  3. Trust gate (cross-angle + geometric sanity) implementado como pure function em `apps/web/lib/canonicalize/sanity.ts` com Wave 0 RED unit tests verde — sanity check correto para a Nailli e85ea7de fixture (5 ok + 1 fallback right_frontal).
  4. Modal redeploy NÃO necessário (canonical é só URLs diferentes; pipeline inalterado). `process/route.ts` resolve `canonical_storage_path` se non-NULL, senão `storage_path`.
  5. Nailli nova (`e85ea7de`) reprocessada pós-canonical produz `vision_features.processing_metadata.detect_diagnostics` com segment centers ≤50 px de distância entre 3 ângulos do mesmo olho — o gate empírico que 07.1.5 perdeu.
...
- [x] 07.1.6-04-PLAN.md — Wave 1: POST /api/capture/canonicalize endpoint (auth gate + ownership + idempotent — chamado por finalize + admin Re-canonicalizar) ✓ 2026-05-12 (0676524; 123 linhas em apps/web/app/api/capture/canonicalize/route.ts; thin wrapper sobre canonicalizeReading; zero new tsc/lint/test regressions)
- [x] 07.1.6-05-PLAN.md — Wave 2: finalizeReadingAction wiring (fire-and-forget canonical fetch before Modal trigger) + process/route.ts resolve canonical_storage_path ?? storage_path ✓ 2026-05-12 (cd81e6e + d417d8a; 2 commits atômicos = +57/-7 LOC em 3 files; D-01 fire-and-forget try/catch never blocks; D-04 env-flag honored downstream; readings.test.ts atualizado para novo 2-fetch contract; zero new tsc/lint/test regressions vs Plan 04 baseline 498/3/24/2)
- [x] 07.1.6-06-PLAN.md — Wave 2: /admin/calibration canonical_status badge + RecanonicalizeButton client component + photos API canonical_url extension ✓ 2026-05-12 (a3b2e58 + e1528b4 + 4e69413; 3 commits atômicos = 1 NEW component (129 linhas RecanonicalizeButton.tsx mirror PhotoDownloadButton) + page.tsx canonical_metadata badge ANTES dos diagnóstico/corpus badges + photos API canonical_url field via 2-batch sign flow com index-matched Map remap; Rule 1 deviation: route.test.ts atualizado para 5-key SignedPhotoEntry shape + new canonical_url test; 499 passed / 3 failed pre-existing Phase 3 debt / 24 todo / 2 skipped (1 new test +0 new failures vs Plan 05 baseline 498); zero new tsc/lint regressions)
- [x] 07.1.6-07-PLAN.md — Wave 3: .gitignore negation (parent re-include + /* wildcard + file-specific negate pattern — Rule 3 fix vs plan literal porque git não negate dentro de ignored parent dir) + probe-haiku-iris-landmarks.mjs header runbook expandido (D-06 role, env vars, cost expectations Sonnet $0.05 / Haiku $0.006, founder pre-PR workflow, refresh workflow) + NAILLI_E85EA7DE_GOLDEN constant rename (e85ea7de post-protocol commit f885462; was NAILLI_DEFAULT 71a7bf1d pre-protocol) + golden fixture (Nailli e85ea7de Sonnet 4.6, 430 lines, $0.047 actual cost) committed via .gitignore negation + probe-canonical.test.ts (173 lines, CANONICAL_PROBE=1 gated, ±5% center_x/y_pct / ±10% radius_pct, mirror byte-exato de ANTHROPIC_INTEGRATION=1 pattern com dynamic import server-only) ✓ 2026-05-12 (629bda4 + 01fcb90 + 1836743 + 93b6d38; 4 atomic commits ~15 min; vitest default 2/2 SKIPPED 0 failures, full suite 499/3/24/2 IDENTICAL a Plan 06 baseline ZERO new failures; tsc/lint clean; **Phase 07.1.6 100% complete — 7/7 plans em 4 waves; ready for /gsd-verify-phase 07.1.6**)
...
- **Cost envelope:** Sonnet ~$0.05/reading × 50 readings/mês = ~$2.5/mês marginal. Fits PROJECT.md envelope (~US$ 100-150/mês total).
- **Anthropic Tier 1 rate limits:** 6 paralelos por reading = 6 RPM. Founder dogfooding peak ≤3 readings/dia → 18 RPM. OK pra Tier 1 (default 5 RPM Sonnet pode ser bottleneck — verificar antes de implementar Promise.all 6-way; planner valida).
- **Sem Modal redeploy:** pipeline inalterado. Canonical é puramente upstream em apps/web.
- **Originais nunca sobrescritos:** canonical_storage_path é additivo. Re-canonicalizar é idempotente.
- **Kill-switch env:** `CANONICAL_CAPTURE_ENABLED=false` força raw → Modal direto sem canonical attempt. Rollback em 1 commit + redeploy.

**UI hint**: yes (pequena extensão de `/admin/calibration` — canonical_status badge + Re-canonicalizar botão).

### Fase 7.2 (INSERTED): Wave C — corpus expansion + threshold tuning
**Origem:** Closure de 07.1-HUMAN-UAT (3 items pendentes após `/gsd-execute-phase 07.1` 2026-05-11 verificar `human_needed` com 24/25 truths). Decimal/inserida em vez de aguardar Fase 8 porque desbloqueia uso real (sem Wave C, Nailli continua classificada `castanho/hematogenea/[]` apesar do código P0.1-P0.4 entregue). **BLOQUEADO em 7.1.5** desde 2026-05-11: investigation revelou que detect/segment garbage upstream impossibilita threshold tuning meaningful — corpus building anota fotos cujo pipeline produz pixel-soup, não LAB-de-íris.
...
**Depends on**: Fase 7.1 (P0.1-P0.4 código entregue; sectoral_pigments wire ativo no Modal payload) + **Fase 7.1.6** (canonical capture entrega inputs geometricamente consistentes pro Modal — bloqueante; substitui dependência prévia em 07.1.5 que ficou B_INFEASIBLE). Pode rodar em paralelo com Fase 7 LLM (audit gates ANCHOR_RE + LGPD-aware já corretos via 07.1-01).
...
- **7.2-E** Add **carúncula lacrimal detection** to canonicalize Sonnet prompt — Sonnet reports carúncula position (left/right/not visible) per image; orchestrator validates against `reading_images.eye` label and flags inverted captures via `canonical_metadata.laterality_mismatch[]`. Rationale: a right-eye photo with carúncula on the wrong side of frame means the founder mounted the iris inverted (or labeled the eye wrong). Without this gate, EVERY downstream analysis on the inverted reading is wrong about sector locations. Scope: extend `apps/web/lib/canonicalize/sonnet-bbox.ts` SYSTEM_PROMPT + parser to capture `eye_landmarks.caruncula_x_pct` or similar; orchestrator (`index.ts`) validates per-image. Touches canonicalize-side, not Modal — placed in 7.2 per founder routing.
...
- [x] 08-02-PLAN.md — lib/asaas/ client REST + webhook-auth timing-safe + zod types + idempotency helper (Wave 1)
...
- [x] 08-13-PLAN.md — Cron daily 4 jobs + vercel.json schedule + CRON_SECRET + middleware /assinatura (Wave 6)
