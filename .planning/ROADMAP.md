...
- **7.2-A** Add findings-hierarchy clause to LLM system prompt — visual prominence (e.g. dense pigmented halo around pupil with heterocromia=true + multi-pigment array) drives section ordering, not just feature presence. Founder UAT showed digestive halo underweighted in §3/§9 ordering. Scope: `apps/web/prompts/system.md` + SPEC.md §6 (D-PR1 frozen contract) coord. NOT 7.2 Modal scope strictly — independent prompt-eng item, but groups with 7.2 deploy cycle.
...
See `.planning/phases/07.1.6-canonical-capture-pipeline/07.1.6-UAT-FINDINGS.md` for the full clinical review.
